...
if (!SERVICE_ROLE_KEY) {
  Deno.test(
    "create-user integration tests skipped — SUPABASE_SERVICE_ROLE_KEY not available",
    () => {
      console.log(
        "Skipping create-user integration tests: SUPABASE_SERVICE_ROLE_KEY is not set in the environment. " +
  ...
  Deno.test(
    "create-user: agent caller is also forbidden (admin-only)",
    async () => {
  ...
  Deno.test(
    "create-user: admin caller with invalid input gets 400",
    async () => {
  ...
  Deno.test(
    "create-user: admin creates a normal user — HTTP + DB side effects",
    async () => {
      const caller = await provisionCaller("admin");
      ...
      try {
        ...
        assertEquals(
          (got.user?.user_metadata as any)?.full_name,
          fullName,
        );
  ...
  Deno.test(
    "create-user: admin creates an agent — role is updated in user_roles",
    async () => {
      const caller = await provisionCaller("admin");
  ...
  Deno.test(
    "create-user: admin creates another admin — role is updated in user_roles",
    async () => {
      const caller = await provisionCaller("admin");
  ...
  Deno.test(
    "create-user: duplicate email returns 400 and does not create a second user",
    async () => {
      const caller = await provisionCaller("admin");
      ...
      try {
        ...
        assertEquals(
          matches.length,
          1,
          "only one auth user should exist for this email",
        );
      } finally {
        await admin.auth.admin.deleteUser(caller.id);
        await deleteUserByEmail(email);
